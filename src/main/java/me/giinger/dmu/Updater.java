...
public class Updater {
	...
	private static final String delimiter = "^v|[\\s_-]v"; // Used for locating
															// version numbers
															// in file names
		...
		try {
			...
			if (createFile) {
				plugin.getLogger().severe(
						"The updater could not create configuration at "
								+ updaterFile.getAbsolutePath());
			} else {
				plugin.getLogger().severe(
						"The updater could not load configuration at "
								+ updaterFile.getAbsolutePath());
			}
	...
	private void saveFile(File folder, String file, String link) {
		...
		try {
			...
			fout = new FileOutputStream(folder.getAbsolutePath()
					+ File.separator + file);
			...
			final File dFile = new File(folder.getAbsolutePath()
					+ File.separator + file);
				...
				if (dFile.isDirectory()) {
					if (this.pluginFile(dFile.getName())) {
						final File oFile = new File(this.plugin.getDataFolder()
								.getParent(), dFile.getName()); // Get current
																// dir
	...
	private boolean versionCheck(String title) {
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			final String localVersion = this.plugin.getDescription()
					.getVersion();
			if (title.split(delimiter).length == 2) {
				final String remoteVersion = title.split(delimiter)[1]
						.split(" ")[0]; // Get the newest file's version number

				if (this.hasTag(localVersion)
						|| !this.shouldUpdate(localVersion, remoteVersion)) {
