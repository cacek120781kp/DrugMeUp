...
public class PlayerListener implements Listener {
			...
			if (stack != null) {
				...
				if ((e.getAction().equals(Action.RIGHT_CLICK_AIR) || e
					...
					if (plugin.isDrug(stack.getTypeId(), data)) {
						...
						plugin.noplace.add(player.getName());
						...
						Bukkit.getScheduler().scheduleSyncDelayedTask(plugin,
								new Runnable() {
									public void run() {
										plugin.noplace.remove(player.getName());
									}
								}, 20);
					}
				}
	...
	public void onEntityDamage(EntityDamageEvent e) {
		if (e.getEntity() instanceof Player)
			if (plugin.isJump.contains(((Player) e.getEntity()).getName()))
	...
	public void gatherEffects(Player p, int i, short dmg) {
		...
		p.sendMessage(Drugs.Colorize(ond));

		plugin.effectlist.clear();
	}

	public void doRandomEffects(Player p) {
		int ii = plugin.effectlist.size();
		int i = gen.nextInt(ii);
	...
	public void pukeInv(Player p) {
		...
		if (plugin.config.getBoolean("Options.EnableEffectMessages") == true) {
			...
			Bukkit.broadcastMessage(Drugs.Colorize(puke));
		}
	...
	public void torchYa(Player p) {
		...
		if (plugin.config.getBoolean("Options.EnableEffectMessages") == true) {
			...
			Bukkit.broadcastMessage(Drugs.Colorize(hot));
		}
		p.setFireTicks(200);
	}
	...
	public void youOd(Player p) {
		...
		if (plugin.config.getBoolean("Options.EnableEffectMessages") == true) {
			...
			Bukkit.broadcastMessage(Drugs.Colorize(death));
		}
		p.setHealth(0);
	}
	...
	public void heartAttack(final Player p) {
		...
		if (plugin.config.getBoolean("Options.EnableEffectMessages") == true) {
			...
			Bukkit.broadcastMessage(Drugs.Colorize(heartAttack));
		}
		plugin.heartattack.add(p.getName());
	...
	public void drunk(final Player p) {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				p.sendMessage(Drugs.Colorize(plugin.config
